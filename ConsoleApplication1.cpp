...
void Jacobi_MPI(int& iteration_count) {
    ...
    for (int i = 1; i <= sqrt(process_count); ++i) {
        if (process_count % i == 0) {
            int j = process_count / i;
    ...
    int start_row = coords_x * local_nx + std::min(coords_x, remainder_x);
    local_nx += (coords_x < remainder_x) ? 1 : 0;

    int local_ny = ny / Py;
    int remainder_y = ny % Py;
    int start_col = coords_y * local_ny + std::min(coords_y, remainder_y);
    local_ny += (coords_y < remainder_y) ? 1 : 0;
    ...
    while (!is_converged) {
        ...
        if (rank < process_count - 1) {
            MPI_Isend(&u[(start_row + local_nx - 1) * ny], ny, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &send_requests[1]);
            MPI_Irecv(&u[start_row + local_nx * ny], ny, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &recv_requests[1]);
        }

        for (int i = start_row + 1; i < start_row + local_nx - 1; ++i) {
            for (int j = start_col + 1; j < start_col + local_ny - 1; ++j) {
    ...
    MPI_Gather(&u[start_row * ny], local_nx * ny, MPI_DOUBLE,
        global_u.data(), local_nx * ny, MPI_DOUBLE,
        0, MPI_COMM_WORLD);

    for (int i = start_row; i < start_row + local_nx; ++i) {
        for (int j = start_col; j < start_col + local_ny; ++j) {
            outfile << (isInEllipse(xmin + i * h_1, ymin + j * h_2) ? u[i * ny + j] : 0);
            if (j < start_col + local_ny - 1) outfile << ",";
        }
        outfile << "\n";
    }
    outfile.close();
